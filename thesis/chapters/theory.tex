...
\noindent
% TODO: better intro
En esta sección repasamos cuestiones fundamentales para resolver problemas de optimización lineales.
Comenzamos estudiando el espacio definido por el conjunto de restricciones $A\vec{x} = \vec{b}$.
\begin{definition}
	...
	\end{equation}
	lo llamamos poliedro. Si, además, $P$ es acotado, entonces decimos que es un politopo.
\end{definition}
Todo poliedro $P$ definido de esta manera representa la intersección de $m$ semi-espacios afinos.
Esto se debe a que
\begin{equation*}
	A\vec{x} \geq \vec{b} \iff \vec{a}_i^T\vec{x} \geq b_i,
\end{equation*}
para toda $1 \leq i \leq m$ y donde $\vec{a}_i$ representa el $i$-ésimo renglón de la matriz $A$.
% TODO: insertar imagen representando un poliedro y un semiespacio afino

% Es posible mostrar que todo poliedro es convexo. Es decir, para cualesquiera dos puntos $\vec{x},
% \vec{y}$ de $P$, es cierto que el segmento de línea que los une también se encuentra en $P$. Si bien
% no lo demostramos aquí, mencionamos que es la consecuencia de mostrar que un semi-espacio afino es
% convexo y que la intersección de conjuntos convexos es convexo.
...
primer lugar, existe un hiperplano afino que pasa por $\vec{x}$ y cuyo semiespacio inducido contiene
completamente al poliedro $P$; en segundo lugar, ningún otro punto de $P$ se encuentra sobre el
hiperplano. Cabe remarcar que este hiperplano no es único.

Sea $P \coloneq \lbrace \vec{x} \in \R^n \colon A\vec{x} \geq \vec{b}\rbrace$ un poliedro y $\vec{c}
\in \R^n$ un vector. Todo problema de optimización de la forma
\begin{equation}
	\label{prim:lineal-opt}
	\max ~ \lbrace \vec{c}^T\vec{x} \colon \vec{x} \in P \rbrace
\end{equation}
entra en una de las siguientes tres categorías:
\begin{enumerate}
	\item El valor óptimo no existe. Esto ocurre porque ningún vector $\vec{x} \in \R^n$ satisface
		el sistema de desigualdades $A\vec{x} \geq \vec{b}$. Es decir, el poliedro $P$ es vacío.
	\item El valor óptimo existe y es infinito. Esto ocurre porque el poliedro $P$ no es acotado y
		somos capaces de encontrar una sucesión de vectores $\lbrace \vec{x}_k \rbrace_{k \in \N}$ que
		satisface $\vec{c}^T\vec{x}_{k+1} > \vec{c}^T\vec{x}_k$ para todo $k \in \N$.
...
\end{enumerate}
% TODO: agregar imagen mostrando los tres casos.
...
\begin{equation*}
	...
	A\vec{x}^- - \vec{s} = \vec{b}, \left(\vec{x}^+, \vec{x}^-, \vec{s}\right) \geq \vec{0}\rbrace,
\end{equation*}
...
objetivo de esta tesis exponer la maquinaria matemática detrás de estos algoritmos\footnote{
	Sin embargo, la literatura para explicar estos métodos es harto abundante.
}. Lo que se encuentra a continuación supone entonces que contamos con un algoritmo para resolver
problemas del tipo \eqref{prerreq:formulation}.
...
relajada, solamente que estamos agregando la restricción de que nuestro vector solución $\vec{x}^*$
sea además entero. Es decir, lo único que cambia es la región de factibilidad. De hecho, si
...
A partir de lo anterior, deducimos inmediatamente que el óptimo $\optilp{z}$ de un programa entero es
una cota inferior del óptimo $z^*$ de su problema relajado, pues ambos son problemas de maximización
y es cierto que $P \cap \Z^n \subseteq P$. 
