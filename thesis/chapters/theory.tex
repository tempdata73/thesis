...
\begin{example}[\cite{fabs}]
	...
	su valor objetivo es $6 < z_2*$, así que también podamos $S_{01}$ por cota.
...
características están interrelacionadas: si hubiéramos resuelto el subproblema $S_{01}$ antes que
$S_{00}$, no habríamos tenido una solución incumbente, así que no lo habríamos podado y
...
numéricos, utilizaremos la implementación de código abierto \emph{COIN-OR Branch and Cut} (CBC)
\textit{solver} de la fundación COIN-OR \cite{coin} para encontrar soluciones a programas lineales
enteros. Otras implementaciones comúnes de código abierto son HiGHS y SCIP, mientras que algunas
implementaciones comerciales son Gurobi Optimizer, IBM ILOG CPLEX Optimizer, y Fico Xpress Solver.
