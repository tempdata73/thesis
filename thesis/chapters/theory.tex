...
\begin{algorithm}[ht]
	...
	\Begin{
		...
		\While{$\mathcal{L} \neq \emptyset$}{
		    ...
		    elegir $x^i_j \not \in \Z$ y generar subproblemas $S_{i1}$ y $S_{i2}$ con
			regiones factibles
		    		$P_{i1} \leftarrow P_i \cup \braces{x_j \leq \floor{x^i_j}}$ y
					$P_{i2} \leftarrow P_i \cup \braces{x_j \geq \ceil{x^i_j}}$, respectivamente\;
