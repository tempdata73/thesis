...
Supondremos en lo que resta de esta tesis que contamos con un algoritmo para resolver problemas del tipo
\eqref{prerreq:formulation}.
\begin{definition}
...
\begin{example}[\cite{fabs}]
	...
	factibles
	\begin{align*}
		S_{0} &\coloneq S \cup \braces{ x_1 \leq \floor{20/7} = 2}, \\
		S_{1} &\coloneq S \cup \braces{ x_1 \geq \ceil{20/7} = 3}.
	\end{align*}
	De la figura \ref{fig:feas}, observamos que $S_{1}$ es vacío y por lo tanto de este problema no
	podemos generar otros subproblemas. En este caso, decimos que \textbf{podamos $S_{1}$ por
	infactibilidad}.

	Ahora bien, la solución al problema $S_{0}$ está dada por $\vec{x}^{(1)} \coloneq (2, 1/2)^T$.
	Encontramos que $x_2^{(1)}$ = 1/2 no es entero y por lo tanto generamos dos nuevos subproblemas:
	\begin{align*}
		S_{00} &\coloneq S_{0} \cup \braces{ x_2 \leq \floor{1/2} = 0}, \\
		S_{01} &\coloneq S_{0} \cup \braces{ x_2 \geq \ceil{1/2} = 1}.
	\end{align*}
	Observemos que la solución $\vec{x}^{(2)}$ de $S_{01}$ es $(2, 1)^T$, la cual es entera y tiene valor
	objetivo $z_2^* \coloneq 7$. No generamos otros subproblemas a partir de este problema porque
	las posibles subregiones factibles estarán contenidas en $S_{01}$ y por lo tanto sus valores objetivos
	serán menores o iguales al de $S_{01}$. Así pues, decimos que \textbf{podamos $S_{01}$ por
	integralidad}.

	La solución de $S_{00}$, en cambio, es $\vec{x}^{(3)} \coloneq (3/2, 0)^T$ y tendríamos que
	...
	subproblema generado a partir de este último problema estára contenido en $S_{00}$, se sigue
	que su valor objetivo será menor o igual al de $S_{00}$. Es decir, no encontraremos otro vector
	cuyo valor objetivo sea mayor que $z_2^*$. Decimos entonces que \textbf{podamos $S_{00}$ por
	cota}.
...
En el algoritmo \ref{algo:bb} de la página \pageref{app:bb} se formalizan estos pasos. Así también,
en el apéndice \ref{app:bb} se contextualiza y detalla el uso de Ramificación y Acotamiento, pues
está continuamente sujeto a extensiones y mejoras por parte de los investigadores en optimización e
investigación de operaciones. Por ello, tendremos en mente el algoritmo \ref{algo:bb} cuando hagamos
referencia a R\&A a menos que mencionemos explícitamente lo contrario.
