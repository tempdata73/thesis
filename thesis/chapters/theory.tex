...
\noindent
Esta sección se divide en tres partes. En primer lugar, damos a conocer las definiciones y
...
\begin{definition}
	...
	coprimo} de $\vec{v}$ si sus entradas son coprimas y si su primera entrada no nula es
	positiva.
\end{definition}
\begin{example}
...
\begin{theorem}
	...
	Entonces la familia de capas enteras $\braces{\qlayer{w}{k} \vcentcolon k \in \Z}$.
\end{theorem}
...
Puesto que nos concentramos en puntos enteros, por el teorema \ref{phase-1:th:cover}
podemos considerar exclusivamente el subconjunto $\braces{\qlayer{q}{k} \vcentcolon k \in \Z}$.
...
Para respetar la restricción presupuestaria \eqref{theory:constraint:budget}, debe ser el caso que
...
el caso $m < 0$ son completamente análogos, donde el orden de las desigualdades cambian o las
funciones piso se transforman en funciones techo, por ejemplo.

En resumen, si $m > 0$, encontramos que las capas enteras que respetan la restricción
