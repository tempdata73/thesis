...
básicos de teoría de números y de programación que constituyen las herramientas principales para la
...
\rbrace$; y el caso $\vec{p} > \vec{0}$. Los siguientes dos capítulos examinarán respectivamente
...
\begin{proof} \hfill
	\begin{enumerate}
		\item
			...
			Entonces, ambos conjuntos $I^+$ e $I^-$ forman una partición del conjunto $\braces{1,
			\ldots, n}$. Podemos escoger escalares positivos $c_1, \ldots, c_n$ que satisfagan
			simultáneamente
			\begin{align}
...
\begin{lemma} \label{lemma:iso1}
	...
	$\vec{\nu} \in \Z^n$ definido en \eqref{eq:vec-omega} satisface $\vec{q}^T\vec{\nu} = 1$.
\end{lemma}
\begin{proof}
...
\begin{theorem}
	\label{th:lattice}
	Supongamos que $q_n \neq 0$. Entonces $\vec{\nu}$ y las columnas de $M$ forman una base
	de la red $\Z^n$.
\end{theorem}
\begin{proof}
...
\begin{proof}
	...
	\begin{equation*}
		\ker{\tilde{M}^T} = \langle \vec{\tvec{q}} \rangle = \langle P\vec{q} \rangle,
	\end{equation*}
...
\begin{observation}
	...
	Sí se cumple que
	\begin{equation*}
		\ker{\tilde{M}^T} = \langle \tvec{q} \rangle = \langle P\vec{q} \rangle
...
como es que $\vec{q}$ descompone el espacio $\Z^n$ a través del vector $\vec{\nu}$ y de la matriz
$M$. Recordemos que $\vec{\nu}$ representa el conjunto de soluciones particulares a estas ecuaciones
