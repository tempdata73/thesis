...
encuentran a lo largo de esta tesis. En primer lugar, la sección \ref{sec:prerreq} recopila
...
resultados. En segundo lugar, la sección \ref{sec:foundations} presenta enunciados y definiciones
...
en algunas ocasiones para profundizar el caso cuando alguna entrada sea nula. Cuando introduzcamos
en el capítulo 4 múltiples restricciones nos desharemos de este supuesto.
...
un máximo elemento. Es decir, el máximo común divisor $d$ siempre está bien definido.
...
La manera más común para calcular el máximo común divisor es a partir del algoritmo de Euclides.
...
en la que deseamos determinar enteros $x_1, \ldots, x_n$ que satisfagan
\begin{equation*}
...
\end{equation}
donde $P \in \R^n$ es un poliedro al cual llamamos \textbf{región factible}, $\vec{c} \in \R^n$ un vector,
...
muestra visualmente esta relación entre semi-espacios afines y poliedros.
...
En el capítulo 3 de \cite{fabs} se muestra que todo poliedro $P \coloneq \lbrace \vec{x} \in \R^n
\colon A\vec{x} \geq \vec{b} \rbrace$ puede ser transformado a la forma estándar
\begin{equation*}
...
$x_i \leq \floor{x_i^*}$ y, similarmente, creamos el nodo hijo derecho añadiendo la restricción $x_i
\geq \ceil{x_i^*}$. Este procedimiento se realiza de manera recursiva. En el apéndice \ref{app:bb}
se presenta de manera más detallada este método.
...
\begin{example}[\cite{fabs}]
	...
	las posibles subregiones factibles estarán contenidas en $S_{001}$ y por lo tanto sus valores objetivos
	serán menores o iguales al de $S_{001}$. Así pues, decimos que \textbf{podamos $S_{001}$ por
	integralidad}.

	La solución de $S_{000}$, en cambio, es $\vec{x}^{(3)} \coloneq (3/2, 0)^T$ y tendríamos que
	ramificar de nuevo en otros dos subproblemas. No obstante, el valor objetivo de este subproblema
	es $z_3^* \coloneq 6$, el cual es menor que $z_2^* = 7$. Como la región factible de cualquier
	subproblema generado a partir de este último problema estára contenido en $S_{000}$, se sigue
	que su valor objetivo será menor o igual al de $S_{000}$. Es decir, no encontraremos otro vector
	cuyo valor objetivo sea mayor que $z_2^*$. Decimos entonces que \textbf{podamos $S_{000}$ por
	cota}.
...
\noindent
Esta sección se divide en cuatro partes. En primer lugar, se dan a conocer las definiciones y
encunciados provistos por \cite{herr}. Es importante aclarar que se tradujeron libremente los
términos \textit{projectively rational vectors} y \textit{c-layers} como ``vectores esencialmente
enteros'' (véase definición \ref{theory:def:rational}) y ``capas enteras'' (véase
\ref{phase-1:def:c-layer}), respectivamente, a falta de encontrar fuentes en español que hicieran
uso de ellos.

En segundo lugar, se muestra una equivalencia entre resolver el problema \eqref{theory:formulation}
y resolver ecuaciones lineales diofantinas. Este resultado se basa en los teoremas
...
teórica para los siguientes capítulos, en particular para el capítulo 4.

Finalmente, se muestra que el vector objetivo $\vec{p}$ del problema original
\eqref{theory:formulation} induce una descomposición de $\Z^n$ y se analiza cómo se relacionan estas
descomposiciones al considerar distintos vectores $\vec{p}$. Esto permitirá mostrar equivalencias
...
el caso $m < 0$ son completamente análogos. Las diferencias recaen en que el orden de las
...
donde $\eta$ está definida en el lema \ref{phase-1:lemma:eta}. Además, por el lema
...
clasificación de este tipo de programas lineales. Hemos visto que el único representante importante
