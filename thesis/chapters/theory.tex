...
al final cuando investiguemos la manera de clasificar este programas lineales enteros.
...
\begin{definition}
	...
	existe un entero $k$ tal que $b = k \cdot a$. También denotamos por $D(a)$ al \textbf{conjunto de
...
divisor entre ellos de manera iterativa:
\begin{equation*}
...
de Euclides, el cual es introducido en la sección 4.5 de \cite{knuth}.
...
de Euclides, el cual es presentado en la sección 4.5 de \cite{knuth}. Observemos que estos coeficientes no son únicos. En efecto, si
...
\emph{simplex} (capítulo 13 de \cite{nocedal}), el cual ``salta'' de vértice en vértice hasta llegar
a uno con valor óptimo. Otros, más modernos y conocidos como \emph{métodos de puntos interiores}
(capítulo 14 de \cite{nocedal}), comienzan en el interior del poliedro $P$ y son ``atraídos'' como
imanes a uno de los vértices con valor óptimo. No es el objetivo de esta tesis exponer la teoría
...
\begin{example}[\cite{fabs}]
	...
	\begin{align*}
		\max_{(x_1, x_2) \in \Z^2} \quad
			...
			& x_1, x_2 \geq 0.
	\end{align*}
	La región factible de este problema se muestra en la figura \ref{fig:feas}. La solución al
	problema relajado, cuya región factible denotamos por $S$, está dada por $\vec{x}^{(0)} \coloneq
	(20/7, 3)$. Como $x_1^{(0)} = 20/7$ no es entero, \textbf{ramificamos binariamente} para generar
	los subproblemas con regiones factibles
	\begin{align*}
...
El algoritmo \ref{algo:bb2} en la página \pageref{algo:bb2} formaliza el procedimiento seguido en
este ejemplo, a excepción de la búsqueda en anchura, pero que es posible implementar con una fila
...
descomposiciones asociadas a distintos vectores $\vec{p}$. Esto permitirá analizar equivalencias
entre distintos programas lineales enteros.
...
\end{definition}
\begin{example}
	El vector $\left(-\sqrt{2}, 1/\sqrt{2}\right) = \frac{1}{\sqrt{2}}(-2, 1)$ es esencialmente entero
...
considerar solamente el caso $m > 0$, ya que las demostraciones y los enunciados para
el caso $m < 0$ son completamente análogos.
% Las diferencias recaen, por ejemplo, en que el orden de
...
\end{theorem}
\begin{proof}
	$(\Rightarrow)$ Supongamos que $\vec{q} \geq \vec{0}$ y $u < 0$. Si $\vec{x} \in \Z_{\geq
	...
	$(\Leftarrow)$ Procedemos por contraposición. Si $u
...
% factible.
El siguiente teorema muestra que nuestro análisis para resolver los problemas
factibles debe dividirse en dos casos.
...
\begin{proof} \hfill
	\begin{enumerate}
		...
		\item Se sigue del teorema anterior que $u \geq 0$. Definamos
			...
			\begin{equation*}
				\paren{\qlayer{q}{k} \cap \R^n_{\geq \vec{0}}} \cap \Z^n = P_k
...
ecuaciones. Será cuestión de los capítulos 2 y 3 obtener soluciones no negativas.
...
\end{equation}
con $\vec{q} = (q_1, \ldots, q_n)$. A lo largo de esta subsección basta suponer que $q_n \neq 0$.
...
Tenemos de \eqref{def:dummy:omega-2} que debemos resolver la ecuación
\begin{equation}
...
\begin{equation}
	...
	\end{cases}
\end{equation}
Sustituyendo en \eqref{eq:x:i} y \eqref{eq:x:last} obtenemos la siguiente proposición original.
\begin{prop}
	\label{prop:xint}
...
\begin{proof}
	...
	\end{equation*}
	Se sigue que las $n - 1$ columnas de $M$ son linealmente independientes.
	Por lo tanto, $M^T$ tiene $n - 1$ renglones linealmente independientes y
	entonces $\dim \ker{M^T} = 1$, así que basta mostrar que $\vec{q} \in
	\ker{M^T}$.
...
\begin{corollary}
	...
	Consideremos el vector $\vec{\nu} \in \Z^n$ definido en
	\eqref{eq:vec-omega} y la matriz $M \in \Z^{n \times (n - 1)}$ definida en
	\eqref{eq:mat-T}, entonces la matriz
	\begin{equation}
		\label{u-matrix}
		U \coloneq [ \vec{\nu} \mid M] \in \Z^{n \times n}
	\end{equation}
...
sus subredes $\Lambda_p$ y $\Lambda_h$, es decir, $\Z^n = \Lambda_p \oplus \Lambda_h$. De esta
...
Dados dos vectores coprimos $\vec{q}, \tvec{q} \in \Z^n$, podemos preguntarnos cómo se relacionan
sus respectivas subredes asociadas. La respuesta a esta cuestión dará comienzo a nuestros intentos
de clasificación de programas lineales enteros, como lo mencionamos en la introducción de esta
tesis. Para ello, debemos especificar el concepto de isomorfismo de redes. Si bien la siguiente
definición no se encuentra en \cite{alex}, estuvo fuertemente inspirada por las equivalencias del
teorema 4.3 y del corolario 4.3a de esa misma referencia.

\begin{definition}
	\label{def:iso}
	Sean $V, W$ dos espacios vectoriales y sea $T\colon V \to W$ una transformación lineal
	invertible cuya matriz asociada es unimodular. Sean $\Lambda_V$ y $\Lambda_W$ dos redes
	respectivas de $V, W$ y supongamos que la imagen de $T\big|_{\Lambda_V}$ es $\Lambda_W$. Entonces
	decimos que $T$ es un \textbf{isomorfismo de redes} y escribimos $\Lambda_V \cong \Lambda_W$.
\end{definition}

Puesto que el símbolo $\cong$ también es utilizado para denotar isomorfismo entre espacios
vectoriales, utilizaremos el calificativo completo de ``isomorfismo de redes'' cuando hablemos de
redes, y diremos ``isomorfismo'' cuando la discusión se centre en espacios vectoriales. Cabe
mencionar que todo isomorfismo de redes también es un isomorfismo de espacios vectoriales, y que el
converso no es necesariamente cierto.

El siguiente teorema es uno de los resultados más importantes de esta tesis, pues muestra que, bajo
isomorfismo de redes, existe una sola clase de equivalencia entre las subredes inducidas por
vectores coprimos con última entrada distinta de cero. Esto sugiere que todos los
programas lineales enteros con la misma dimensión son equivalentes entre sí.
\begin{theorem}
	\label{th:equiv}
	Sean $\vec{q}, \tvec{q} \in \Z^n$ vectores coprimos con últimas entradas no nulas, y sean
	respectivamente $\Lambda_p, \Lambda_h$ y $\tilde{\Lambda}_p, \tilde{\Lambda}_h$ las subredes
	inducidas por ellos (ver \eqref{eq:dec}). Entonces $\Lambda_p \cong \tilde{\Lambda_p}$ y
	$\Lambda_h \cong \tilde{\Lambda}_h$.
\end{theorem}
\begin{proof}
	Consideremos la matriz $U = [\vec{\nu} | M]$ inducida por $\vec{q}$ de acuerdo al corolario
	\ref{cor:unimodular} y definida en \eqref{u-matrix}. Por este último corolario y por la
	definición \ref{def:iso}, tenemos que $U$ testifica el isomorfismo de redes entre $\Z^n$ y
	$\Lambda_p \oplus \Lambda_h$. Similarmente, la matriz $\tilde{U} = [\tvec{\nu} | \tilde{M}]$
	inducida por $\tvec{q}$ testifica el isomorfismo de redes entre $\Z^n$ y $\tilde{\Lambda}_p
	\oplus \tilde{\Lambda}_h$. Ahora bien, por cómo están construidas en \eqref{u-matrix}
	encontramos que
	\begin{subequations}
		\begin{align}
			\tvec{\nu} &\overset{\tilde{U}^{-1}}{\longmapsto} \vec{e}_1 \overset{U}{\longmapsto}
			\vec{\nu}, \label{eq:equiv:1} \\
			\tilde{M} &\overset{\tilde{U}^{-1}}{\longmapsto} [\vec{e}_2 \mid \cdots \mid \vec{e}_n]
			\overset{U}{\longmapsto} M \label{eq:equiv:2}.
		\end{align}
	\end{subequations}

	Como $\tvec{\nu}$ y $\vec{\nu}$ son bases respectivas de $\Lambda_p$ y $\tilde{\Lambda}_p$, se
	sigue que la imagen de $U\tilde{U}^{-1}\big|_{\tilde{\Lambda}_p}$ es $\Lambda_p$. Análogamente,
	tenemos que la imagen de $U\tilde{U}^{-1}\big|_{\tilde{\Lambda}_h}$ es $\Lambda_h$. También
	tenemos por el corolario \ref{cor:unimodular} que $U\tilde{U}^{-1}$ es invertible y unimodular:
	\begin{align*}
		(U\tilde{U}^{-1})(\tilde{U}U^{-1}) &= U(\tilde{U}^{-1}\tilde{U})U^{-1} = UI_nU^{-1} =
		UU^{-1} = I_n, \\
		\det{(U\tilde{U}^{-1})} &= \det{(U)}\det{(\tilde{U}^{-1})} = \det{(U)}/\det{(\tilde{U})} = \pm 1.
	\end{align*}
	Se sigue de la definición \ref{def:iso} que $\tilde{\Lambda}_p \cong \Lambda_p$ y
	$\tilde{\Lambda}_h \cong \Lambda_h$.
\end{proof}

Debido a este teorema podemos expresar un programa lineal entero en términos de otro, posiblemente
más sencillo de resolver, a partir de un cambio de variables. Por ejemplo, el teorema
\ref{th:multeq} del capítulo 4 muestra explícitamente este cambio de variables para ``alinear'' el
vector $\vec{q}$ al vector canónico $\vec{e}_1$. Una consecuencia de esto será el teorema
\ref{th:bbspeed}, el cual muestra que este problema transformado permite a R\&A
podar lo más tempranamente posible algunos subproblemas infactibles. Así también, en el siguiente
capítulo haremos uso implícito de este teorema para permutar las entradas del vector $\vec{q}$ de
manera que podamos resolver el problema \ref{theory:formulation} en tiempo cuadrático (véase la
subsección \ref{subsec:inf:complex}) cuando una de las entradas de $\vec{q}$ es negativa. El
siguiente corolario es una extensión natural del teorema anterior.

\begin{corollary}
	\label{cor:iso3}
	Sean $\vec{q}, \tvec{q} \in \Z^n$ vectores coprimos con últimas entradas no nulas, y
	sean $M$ y $\tilde{M}$ sus respectivas matrices definidas en \eqref{eq:mat-T}, entonces
	\begin{align*}
...
\end{corollary}
\begin{proof}
	Se sigue inmediatamente de \eqref{eq:equiv:2}, ya que la independencia lineal de las columnas de
	$M$ se mantiene bajo la premultiplicación de $\tilde{M}$ por la matriz $U\tilde{U}^{-1}$, pues
	esta es invertible, de acuerdo a la demostración del teorema \ref{th:equiv}.
\end{proof}

A continuación enfocamos nuestra atención al caso particular de permutaciones del vector $\vec{q}$.
La motivación principal de esto se debe a que los resultados principales del siguiente capítulo se
fundamentan en este caso.

Observemos que si $q_n = 0$, es válido permutar las entradas de $\vec{q}$ de manera que el vector
permutado $\tvec{q}$ cumpla el supuesto $\tilde{q}_n \neq 0$. El corolario \ref{cor:iso3} responde a
la pregunta de cómo se relacionan los espacios fundamentales asociados a las matrices $M$ y
$\tilde{M}$ inducidas por $\vec{q}$ y $\tvec{q}$ respectivamente. Pero si $q_n = 0$, entonces puede
ser que $M$ no esté bien definida. Lo que se encuentra a continuación también nos permite justificar
por qué es posible ignorar las entradas nulas del vector $\vec{q}$.
...
% 	\begin{equation*}
% 		\ker{M^T} = \img\braces{M}^\perp = \gen\braces{\vec{q}} \cong \gen\braces{\tvec{q}} =
% 		\img\braces{\tilde{M}}^\perp = \ker{\tilde{M}^T}.
% 	\end{equation*}
% \end{proof}
% \begin{observation}
...
\end{lemma}
\begin{proof}
	$(\Rightarrow)$ Supongamos que $\orb(\tvec{q}) = \orb(\vec{q})$. Debemos mostrar que $\tvec{q} \in
	...
	$(\Leftarrow)$ Supongamos que $\tvec{q} \in \orb(\vec{q})$, luego existe una matriz de permutación $P \in \Z^{n\times
...
El teorema \ref{th:equiv} nos permite identificar la descomposición de $\Z^n$ a partir de
$\orb(\vec{q})$ y no solamente de $\vec{q}$. Ahora bien, supongamos que $\vec{q} \in \Z^{n + \ell}$
es un vector coprimo con $\ell$ entradas nulas. Definamos el conjunto ordenado de índices no nulos
de $\vec{q}$ como $\sigma = (i \vcentcolon q_i \neq 0)$, y también definamos la proyección $\proy{n
+ \ell} \colon \Z^{n + \ell} \to \Z^{n}$ a partir de $\proy{n + \ell}(\vec{q})_i \coloneq
q_{\sigma_i}$. Luego, $\proy{n + \ell}(\vec{q})$ es un vector coprimo con entradas distintas de
cero. Por el teorema \ref{th:lattice} (y también por la discusión que le sucede), sabemos que
$\proy{n + \ell}(\vec{q})$ induce la descomposición $\Z^n = \Lambda_p \oplus \Lambda_h$, donde
$\Lambda_p$ y $\Lambda_h$ están definidas en \eqref{eq:dec}. Pero una de las consecuencias del
teorema \ref{th:equiv} es que todo vector $\tvec{q} \in \orb(\proy{n + \ell}(\vec{q}))$ induce una
descomposición de redes isomorfa. Esto justifica el hecho de que podamos tanto ignorar las entradas nulas
como permutar todas las entradas del vector coprimo $\vec{q}$, y también motiva el siguiente resultado.
\begin{theorem}
	\label{th:isoclass}
...
\begin{proof}
	...
	$\proy{n}(\vec{q}) = \vec{q}$. Entonces $\tvec{q}$ tiene $m - n$ entradas nulas, pues de otra
	...
	$\proy{m}(\tvec{q}) \in \orb(\vec{q})$. Finalmente, del teorema \ref{th:equiv} obtenemos lo que
	queríamos demostrar.
\end{proof}

Hemos visto que el teorema \ref{th:equiv} nos permite definir una única clase de equivalencia entre
programas lineales enteros siempre y cuando las últimas entradas de los vectores coprimos asociados
a los vectores objetivo sean distintos de cero. No obstante, computar el cambio de variable entre
cualesquiera dos programas lineales enteros puede ser costoso tanto en tiempo como en espacio. Otro
candidato interesante para definir las clases de equivalencia son las órbitas. Aunque no lo
mostraremos en esta tesis para evitar extendernos más allá de lo necesario, esto tiene su causa en
que las matrices de permutación forman un subgrupo del grupo de matrices ortogonales sobre los
enteros y, por lo tanto, es computacionalmente más sencillo determinar el cambio de variable que
facilita la resolución del problema.

\begin{definition}
	Sean $\vec{p} \in \R^n$ y $\tvec{p} \in \R^m$ vectores esencialmente enteros
	(ver definición \ref{theory:def:rational}). Entonces decimos que $\vec{p}$ y $\tvec{p}$ son
	\textbf{equivalentes} si $\orb(\proy{n}(\vec{q})) = \orb(\proy{m}(\tvec{q}))$, donde $\vec{q}$ y
	$\tvec{q}$ son sus respectivos múltiplos coprimos. En este caso escribimos $\vec{p} \sim
	\tvec{p}$.
\end{definition}
Puesto que el múltiplo coprimo de un vector esencialmente entero es único esta relación está bien
definida. Luego, por las propiedades de la igualdad, tenemos que esta relación es una de
equivalencia sobre el conjunto de vectores esencialmente enteros, independientemente de su
dimensión.
...
\eqref{theory:formulation} es esencialmente el mismo. Esto permite clasificar los programas lineales
enteros de manera que determinar el cambio de variable entre ellos no sea tan costoso computacionalmente.
...
asociado $\vec{q} \in \Z^n$ cumpla $q_{n-1} < 0 < q_n$. Esto ilustrará concretamente la
