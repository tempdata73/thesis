...
$0 \leq \hat{u} \leq u$.
Si $\hat{u} < p_m$, entonces $f_{m-1}(\hat{u})$ es el valor óptimo, pues $p_m > 0$
y buscamos una solución no negativa. De \eqref{dp:rec} se sigue que $f_m(\hat{u})$ es el valor
óptimo para todo $0 \leq \hat{u} < p_m$. Luego, aprovechando la hipótesis inductiva sobre $m$ junto
este último punto, podemos utilizar un argumento fuertemente inductivo sobre $\hat{u}$ para mostrar
que $f_m(\hat{u})$ también es el valor óptimo para todo $p_m \leq \hat{u} \leq u$. De esta manera
encontramos que si $f_{m - 1}(\hat{u})$ es el valor óptimo para todo $0 \leq \hat{u} \leq u$,
entonces $f_m(\hat{u})$ también lo es. Por el caso base $m = 1$ tenemos entonces que $f_m(\hat{u})$
es el valor óptimo para todo $1 \leq m \leq n$ y para todo $0 \leq \hat{u} \leq u$.
...
terminan con la solución óptima, así que la implementación realizada por el autor de esta
formulación dinámica incluye una manera de construir la solución, a pesar del costo adicional en
eficiencia, el cual es $\mathcal{O}(u)$.
