...
\noindent
Inspirados por el teorema \ref{theory:th:feasibility}, en este capítulo analizamos el caso en el que
el vector coprimo $\vec{q}$ tiene entradas estrictamente positivas. Bajo este supuesto adicional, el
problema \eqref{theory:formulation} es una instancia particular del famoso \emph{Problema de la
Mochila}:
\begin{subequations}
...
considerado como un \emph{Problema de la Suma de Conjuntos no Acotado}. Referimos a los capítulos 3 y 4 de
\cite{martello} para una descripción más detallada de estos problemas.
...
Debido al supuesto $\vec{q} > \vec{0}$, observemos de \eqref{dummy:eq:ith-equation}:
\begin{equation*}
	...
	= \omega_i,
\end{equation*}
que podemos agregar la condición $\omega_{i} \geq 0$, en efecto, buscamos que $\vec{x}$ sea no
negativo y recordemos que $g_i$ es un máximo común divisor (ver \eqref{dummy:eq:ith-g}), por lo que
es estrictamente positivo. Juntando esto con el supuesto $\vec{q} > \vec{0}$, encontramos que
...
\end{lemma}
\begin{proof}
	Sea $i^* \coloneq \argmax\braces{q_1, \ldots, q_n}$ y consideremos el vector
	\begin{equation*}
	...
	entera $\qlayer{\vec{q}}{k}$ con $k \in \lbrace \tau, \tau + 1, \ldots \rbrace$
...
\end{observation}
Vimos en el primer capítulo que, si $m < 0$, entonces existe un parámetro $\eta'$ análogo a $\eta$.
En este caso, también
aquí existe $\tau' \geq \eta'$ tal que la solución del problema \eqref{theory:formulation} se
encuentra en una capa parametrizada por $\eta' \leq k \leq \tau'$.
...
\begin{proof}
	...
	\begin{align*}
		...
		- \left\lfloor \left\lfloor \frac{x}{q} + \frac{\lcm{q,m}}{q} \right\rfloor \frac{q}{m}
			\right\rfloor,
	\end{align*}
	pero $q, m \mid \lcm{q, m}$, y por las propiedades de la función piso obtenemos: 
	\begin{align*}
		\Delta(x + \lcm{q,m})
		&=
...
múltiplo $m$ sea entero. Aunque no lo demostraremos, es nuestra creencia que este resultado puede
ser generalizado para $m$ racional. Independientemente del comportamiento periódico de $\Delta^*$,
tenemos que esta función varía significativamente ante cambios en $m$. Esto último implica que el
número de capas enteras a revisar depende del número de cifras decimales utilizadas para especificar
$\vec{p}$.
...
%     \centering
%     \includegraphics[width=\linewidth]{/home/sebastian/repos/thesis/static/misc/delta8m12q.pdf}
%   \end{minipage}
%   \hfill
%   \begin{minipage}[t]{0.48\textwidth}
%     \centering
%     \includegraphics[width=\linewidth]{/home/sebastian/repos/thesis/static/misc/delta008m960q.pdf}
%   \end{minipage}
% 
...
Luego, observamos que el ``trozo'' no negativo de una capa entera $\qlayer{\vec{q}}{k}$ crece a medida que $k$ aumenta. Recordemos que $k \leq
...
$\qlayer{\vec{q}}{k}$ con el problema \eqref{theory:formulation}. Así pues, concluiremos
...
\begin{definition}
	...
	\textbf{bola cerrada} sobre la $k$-ésima capa entera $\qlayer{\vec{q}}{k}$ con
	radio $r > 0$ y centro $\vec{x} \in \qlayer{\vec{q}}{k}$ como
	\begin{equation}
		\label{eq:k-ball}
		B_r^{(k)}(\vec{x}) \coloneq \lbrace \vec{y} \in \R^n \colon \norm{\vec{y} - \vec{x}} \leq r
	\rbrace \cap \qlayer{\vec{q}}{k}.
	\end{equation}
\end{definition}
...
\begin{theorem}
	...
	\begin{equation*}
		\left\lbrace B_r^{(k)}(\vec{x}) \colon \vec{x} \in \qlayer{\vec{q}}{k} \cap
			\Z^n \right\rbrace
	\end{equation*}
	es una cubierta de la $k$-ésima capa entera $\qlayer{\vec{q}}{k}$.
\end{theorem}
\begin{proof}
	Como $q_n \neq 0$, recordemos del teorema \eqref{th:lattice} que
	\begin{equation*}
		\vec{x} \in \qlayer{\vec{q}}{k} \cap \Z^n \iff \vec{x} = k\vec{\nu} + M\vec{t}
	\end{equation*}
	...
	\qlayer{\vec{q}}{k}$ para todo $r > 0$ y para todo $\vec{t} \in \Z^{n-1}$. Luego, para
	...
	\end{equation}
	encontramos que $\vec{y} \in B_r^{(k)}(\vec{x})$. Luego, como $\vec{y} \in \qlayer{\vec{q}}{k}$
	fue genérico, tenemos que si $r$ está definido por \eqref{eq:radius},
	entonces
	\begin{equation}
		\label{eq:ball-cover:2}
		\qlayer{\vec{q}}{k} \subseteq
		\bigcup_{\vec{t} \in \Z^{n-1}}B_r^{(k)}(k\vec{\nu} + M\vec{t}).
	\end{equation}
	Juntando esto con \eqref{eq:ball-cover:1} obtenemos lo que queríamos demostrar.
\end{proof}
...
lemas y teoremas fueron realizadas de manera independiente.
\begin{definition}[\cite{boyd}]
	\label{def:aff}
...
\begin{lemma}
	...
	para todo $1 \leq j \leq m$ se tiene que
	\begin{equation*}
		\aff\braces{\vec{v}_1, \ldots, \vec{v}_m} = \vec{v}_j + \gen\braces{\vec{v}_i -
		\vec{v}_j}_{i=1}^{m} = \vec{v}_j + \gen\braces{\vec{v}_i - \vec{v}_j}_{i\neq j}.
	\end{equation*}
\end{lemma}
\begin{proof}
	Puesto que $\vec{v}_j - \vec{v}_j = \vec{0}$ es linealmente dependiente de los vectores
	$\braces{\vec{v}_i - \vec{v}_j}_{i \neq j}$, se sigue que
	\begin{equation*}
		\vec{v}_j + \gen\braces{\vec{v}_i - \vec{v}_j}_{i = 1}^{m}
		=
...
\begin{example}
	...
	entera $\qlayer{\vec{q}}{k}$ es la combinación afín de un conjunto de vectores.
	...
	obtenemos
	\begin{equation*}
		\qlayer{\vec{q}}{k} = \vec{u}_j + \gen\braces{\vec{u}_i - \vec{u}_j}_{i \neq
		j}.
	\end{equation*}
...
\begin{lemma}
	...
	Sea $\vec{q} > \vec{0}$ un vector coprimo y sea $\qlayer{\vec{q}}{k}$ la
	...
	\begin{equation*}
		\sigma = \qlayer{\vec{q}}{k} \cap \R^n_{\geq \vec{0}}.
	\end{equation*}
\end{lemma}
\begin{proof}
	($\subseteq$) En el ejemplo \ref{ex:aff} mostramos que
	\begin{equation*}
		\qlayer{\vec{q}}{k} = \aff\braces{\vec{u}_1, \ldots, \vec{u}_n},
	\end{equation*}
	y como $\sigma = \conv\braces{\vec{u}_1, \ldots, \vec{u}_n}$, automáticamente tenemos de
	\eqref{aff:contention} que $\sigma \subseteq \qlayer{\vec{q}}{k}$, así que falta mostrar que
	$\sigma \subseteq \R^n_{\geq \vec{0}}$. Sea $\vec{x} \in \sigma$, luego existen escalares
	$\theta_1, \ldots, \theta_n$ no negativos tales que
	\begin{equation*}
	...
	implica que $\vec{x} \in \qlayer{\vec{q}}{k} \cap \R^n_{\geq \vec{0}}$.

	($\supseteq$) Como $\vec{x} \in \qlayer{\vec{q}}{k}$, existen escalares $\theta_1, \ldots,
	\theta_n$ tales que
	\begin{equation*}
		\vec{x} = \theta_1\vec{u}_1 + \cdots + \theta_n\vec{u}_n,
	\end{equation*}
	y que satisfacen $\theta_1 + \cdots + \theta_n = 1$. Por la definición de $\sigma$, basta
	mostrar que $\theta_i \geq 0$ para toda $1 \leq i \leq n$. Puesto que $\vec{x} \geq \vec{0}$
	tenemos por la ortogonalidad de $\braces{\vec{u}_i}_{i=1}^{n}$ que
	\begin{equation*}
		0 \leq \frac{k}{q_i}x_i = \vec{u}_i^T\vec{x} = \norm{\vec{u}_i}^2\theta_i,
	\end{equation*}
	y como $\norm{\vec{u}_i}^2 \geq 0$ se sigue que $\theta_i \geq 0$ para toda $1 \leq i \leq n$.
\end{proof}
...
$\braces{\vec{u}_i}_{i=1}^{n}$ son todos ortogonales entre sí, a partir de la definición
\ref{def:barycenter} y de la ecuación \eqref{def:u-basis} obtenemos los siguientes cálculos:
\begin{align*}
...
\begin{equation}
	\label{eq:facetaux}
	\vec{\mu}_j^T\est{\sigma}_j = -\frac{k^2}{\norm{\vec{q}}^2},
\end{equation}
y por lo tanto
\begin{equation}
	\label{eq:signed}
...
\begin{proof}
	...
	($\subseteq$) Sea $\vec{x} \in \sigma$. Por el lema \ref{lemma:sigmachar1} sabemos que $\vec{x}
	\in \qlayer{\vec{q}}{k}$. También sabemos, por el ejemplo \ref{ex:aff}, que existen escalares no
	...
	\begin{align*}
		\vec{\mu}_j^T\vec{x}
		&=
		\paren{\vec{u}_j - \frac{k}{\norm{\vec{q}}^2}\vec{q}}^T
		\sum_{i=1}^{n}\theta_i\vec{u}_i \\
		&=
		\vec{u}_j^T\paren{\theta_j\vec{u}_j + \sum_{i \neq j}\theta_i\vec{u}_i}
		- \frac{k}{\norm{\vec{q}}^2}\sum_{i=1}^{n}\theta_i\vec{q}^T\vec{u}_i \\
		&=
		\theta_j\norm{\vec{u}_j}^2
		- \frac{k}{\norm{\vec{q}}^2}\sum_{i=1}^{n}\theta_ik \\
		&=
	...
	\begin{equation*}
		...
		=
		\theta_j\norm{\vec{u}_j}^2 \geq 0,
	\end{equation*}
	para todo $1 \leq j \leq n$.

	($\supseteq$) Mostramos por contradicción que si $\vec{x}$ no se encuentra en el lado izquierdo
	de \eqref{eq:sigmachar2}, entonces tampoco se encuentra en el lado derecho. Así pues, supongamos
	que $\vec{x} \not\in \sigma$. Por el lema \ref{lemma:sigmachar1} se sigue o bien que $\vec{x}
	\not\in \qlayer{\vec{q}}{k}$ o bien que $\vec{x} \not\in \R^n_{\geq \vec{0}}$. En el primer caso
	obtenemos inmediatamente que $\vec{x}$ no se encuentra en el lado derecho de
	\eqref{eq:sigmachar2}. Luego, como $\vec{x} \not \in \sigma$, es válido suponer que $\vec{x} \in
	\qlayer{\vec{q}}{k}$ pero $\vec{x} \not\in \R_{\geq \vec{0}}$. Como
	$\braces{\vec{u}_i}_{i=1}^{n}$ es base de $\R^n$, existen escalares
	$\braces{\lambda_i}_{i=1}^{n}$ tales que
	\begin{equation*}
		\vec{x} = \sum_{i=1}^{n}\lambda_i\vec{u}_i.
	\end{equation*}
	Como las entradas de $\vec{u}_1, \ldots, \vec{u}_n$ son todas no negativas y $x_j < 0$ para
	alguna $1 \leq j \leq n$, se sigue que $\lambda_j < 0$. Observemos que
	\begin{align*}
		\vec{\mu}_j^T\vec{x}
		&=
	...
	\end{align*}
	Pero $\vec{x} \in \qlayer{\vec{q}}{k} = \aff\braces{\vec{u}_1, \ldots, \vec{u}_n}$
	(véase el ejemplo \ref{ex:aff}) y entonces los escalares $\lambda_1, \ldots, \lambda_n$ suman a 1.
	Sustituyendo,
	\begin{equation*}
		\vec{\mu}_j^T\vec{x} = \lambda_j\norm{\vec{u}_j}^2 - \frac{k^2}{\norm{\vec{q}}^2},
	\end{equation*}
	retomando el valor de $\vec{\mu}_j^T\est{\sigma}_j$ en \eqref{eq:facetaux}, encontramos que
	\begin{equation*}
		\vec{\mu}_j^T(\vec{x} - \est{\sigma}_j) = \lambda_j\norm{\vec{u}_j}^2 = \lambda_j\frac{k^2}{q_j^2} < 0
	\end{equation*}
	y entonces $\vec{x}$ no es elemento del semi-espacio afín $\braces{\vec{x} \vcentcolon
...
distancias $d(\est{\sigma}, \sigma_j)$ para cada $1 \leq j \leq n$. El siguiente teorema relaciona
estas distancias con el radio de la bola inscrita en el símplice $\sigma$.
\begin{theorem}
...
\begin{proof}
	...
	\end{equation*}
	daod que supusimos $r \leq d(\est{\sigma}, \sigma_j)$ para todo $1 \leq j \leq n$. Además, como
	$\vec{x} \in B_r^{(k)}(\est{\sigma})$, tenemos por la definición \ref{fin:def:ball} que $\vec{x}
	\in \qlayer{\vec{q}}{k}$. De lo anterior obtenemos
	\begin{equation*}
		\vec{x} \in
		\bigcap_{j=1}^{n}
		\braces{\vec{x} \in \R^n \colon \uvec{\mu}_j^T(\vec{x} - \est{\sigma}_j) \geq 0}
		\cap \qlayer{\vec{q}}{k} = \sigma,
	\end{equation*}
	...
	n \rbrace$. Como $\sigma_j$ es cerrado, existe un punto $\vec{x} \in \sigma_j$ que satisface
	$d(\est{\sigma}, \sigma_j) = d(\est{\sigma}, \vec{x})$. Luego, $\norm{\vec{x} - \est{\sigma}} <
	r$. Entonces existe $\varepsilon > 0$ tal que
	\begin{equation*}
	...
	afín definido por $\braces{\vec{x} \vcentcolon \uvec{\mu}^T(\vec{x} - \est{\sigma}_j) \geq 0}$. Así pues,
...
y cada una de sus facetas $\sigma_j$. Aplicando el teorema anterior, podemos obtener una
...
Recordemos que la motivación detrás de todos estos cálculos se fundamenta en determinar la
existencia de un punto entero sobre el ``trozo'' no negativo de la $k$-ésima capa entera. Esto
permitirá determinar la existencia de soluciones enteras no negativas de la ecuación lineal
diofantina $\vec{q}^T\vec{x} = k$ y por lo tanto de obtener un resultado análogo al teorema
\ref{infinite:th:complexity}.

Para empezar, en el siguiente teorema se proporciona una cota
inferior de manera que podamos asegurar la existencia de puntos enteros en una vecindad del
...
$k$. El punto más interesante es aquel que provee la cota inferior más pequeña. Aunque no lo
demostraremos, creemos que el baricentro $\est{\sigma}$ provee la mejor cota.
...
\begin{proof}
	...
	$\vec{x} \geq \vec{0}$ y además $\vec{x} \in \qlayer{\vec{q}}{k}$ por el lema
...
\begin{proof}
	...
	$\eta$ encontramos que $\vec{x}$ también es un punto óptimo. Así, solo es
	necesario resolver una ecuación lineal diofantina para determinar la solución del problema
	\eqref{theory:formulation}.
\end{proof}

El teorema \ref{th:intnonneg1} provee nuevas cotas superiores para el número de Frobenius en el
Problema Diofantino de Frobenius \cite{frob}. De manera resumida, dada una colección de enteros
$q_1, \ldots, q_n$ coprimos, el número de Frobenius es el entero $F$ más grande tal que $F$ no pueda
ser expresado como una combinación lineal entera no negativa de $q_1, \ldots, q_n$. En efecto, o
bien $F$ es mayor o igual que el lado derecho de \eqref{eq:eta-limit} o bien es estrictamente menor.
El primer caso no puede ocurrir porque el teorema \ref{th:intnonneg1} asegura que la ecuación lineal
diofantina $\vec{q}^T\vec{x} = F$ tiene una solución no negativa, es decir, que $F$ puede ser
escrito como una combinación lineal entera no negativa de $q_1, \ldots, q_n$, pero entonces $F$ no
puede ser el número de Frobenius de estos enteros. Así pues, $F$ debe ser estrictamente menor que el
lado derecho de \eqref{eq:eta-limit}. Un estudio sobre cómo se compara esta colección de cotas con
respecto a las presentadas en el capítulo 3 de \cite{frob}, por ejemplo, queda fuera del propósito
de esta tesis.

En último lugar, mencionamos que eventualmente es suficiente con revisar la primera capa entera. Sin
embargo, no hemos demostrado que el número de capas enteras a revisar eventualmente decrece en
cuanto el presupuesto $u$ aumenta. Demostrar que este comportamiento siempre se cumple es mucho más
difícil y queda fuera del propósito de esta tesis.
...
escalar $m$ que satisface $\vec{p} = m\vec{q}$ es también positivo. Ya hemos visto cómo la
solución del problema \eqref{theory:formulation} se traduce a la búsqueda de una solución entera no
negativa de la ecuación lineal diofantina $\vec{q}^T\vec{x} = k$ para alguna $k \leq \eta$, donde
$\eta$ es tomada del lema \ref{phase-1:lemma:eta}.

En esta sección presentamos los algoritmos \ref{algo:fin:helper} y \ref{algo:fin:dioph} en las
páginas \pageref{algo:fin:helper} y \pageref{algo:fin:dioph}, que se encargan de obtener soluciones
enteras no negativas de la ecuación $\vec{q}^T\vec{x} = k$. De manera resumida, el algoritmo
\ref{algo:fin:helper} utiliza la estrategia de ``vuelta atrás'' (\textit{backtracking}) para
buscar una solución entera no negativa de la ecuación anterior. En caso de que este algoritmo no
encuentre ninguna solución, el algoritmo \ref{algo:fin:dioph} desciende a la $(k - 1)$-ésima capa
entera para volver a realizar la búsqueda.
...
En realidad, sabemos por el lema \ref{lemma:tau} que el parámetro $k$ definido en la línea
\ref{alg:fin:loop} descenderá hasta 0 si y solo si el parámetro $\tau$ definido en \eqref{eq:tau}
es nulo. No obstante, la demostración del teorema \ref{th:fin:dioph:correct} se vuelve más simple
cuando en el algoritmo \ref{algo:fin:dioph} dejamos que $k$ se encuentre en $[0, \eta]$ en vez de
$[\tau, \eta]$.
Esta modificación no afecta en lo más mínimo la correctud del algoritmo.

Vale la pena mencionar lo siguiente con respecto al algoritmo \ref{algo:fin:helper}. Varios
lenguajes de programación, tales como Python, cuentan con un límite en las llamadas de recursión que
el usuario puede realizar. Si bien este límite puede modificarse, aumenta la posibilidad de
encontranos con un desbordamiento de pila, pues el algoritmo \ref{algo:fin:helper} no está expresado
en forma de recursión terminal.

Además, este algoritmo no minimiza, por ejemplo, el número de llamadas para calcular el máximo común
divisor en la línea \ref{alg:fin:g} o los coeficientes de Bézout en la línea \ref{alg:fin:bez2}. En
efecto, supongamos que un intervalo de factibilidad finito $I$ definido en la línea
\ref{alg:fin:feas} causa que $\vec{x}^{\texttt{tail}}$ sea \NIL~ para todo $t \in I$. Entonces
estaríamos haciendo $|I|$ llamadas recursivas a \texttt{NonNegativeIntSolFin} en la línea
\ref{alg:fin:subeq} con el mismo vector $\vec{q}^{\texttt{tail}}$ y, por lo tanto, estaríamos
calculando $|I|$ veces la misma $g$ y los mismos $x', \omega'$.

A pesar de los puntos anteriores, decidimos escribir el algoritmo \ref{algo:fin:helper} de esa
manera debido a que se simplificaba de manera significativa la demostración del teorema
\ref{th:fin:helper:correct}. Realizamos una implementación equivalente más eficiente a través de
ciclos para obtener los resultados de la siguiente sección.
...
de Ramificación y Acotamiento (R\&A) implementado en el \textit{solver} COIN-OR Branch and Cut
(CBC) (\cite{coin}). Finalmente, explicamos y realizamos nuestros experimentos numéricos.

Utilizamos el mismo equipo así como la misma configuración de CBC que se detalla en la página
\pageref{subsec:inf:res} para la realización de estos experimentos.
...
correctud de nuestro método. En esta sección preferimos realizar una implementación equivalente en ciclos
...
La implementación por ciclos utiliza una pila (\textit{stack}) de estados $(i, b_1, b_2, \omega)$,
donde $i$ indica el nivel o la variable $t_i$ que debemos escoger; el resto de los parámetros están
definidos en el algoritmo \ref{algo:fin:helper}. La elección de $t_i$ ciertamente es la más
importante, pues determina el intervalo de factibilidad $I = [b_1, b_2]$ en el siguiente nivel $i +
1$. Existen, \textit{a priori}, dos estrategias que podemos adoptar para este tipo de elecciones.
...
árbol (\textit{depth-first search}).

Encontramos en los experimentos preliminares que la segunda estrategia es mucho más eficaz. En
los experimentos de la subsección \ref{subsec:exp:fin:n}, la primera estrategia tenía tiempos de
terminación aproximadamente equivalentes a los de CBC. Para los experimentos de la subsección
\ref{subsec:exp:fin:rhs}, ambas estrategias tenían tiempos de terminación significativamente menores
que CBC. Por ello, decidimos omitir los resultados de la primera estrategia.

Observemos que el orden en el que consideramos los estados $(i, b_1, t_i - 1, \omega)$ y $(i, t_i +
1, b_2, \omega)$ determina si visitamos primero el lado izquierdo o el lado derecho del árbol.
Realizamos los experimentos con ambas posibilidades. Así pues, llamaremos \texttt{dioph\_left} a la
implementación que recorre primero el subintervalo izquierdo y definimos análogamente
\texttt{dioph\_right}.
...
El siguiente teorema muestra la correctud de esta formulación de programación dinámica. Aunque no
las realizaremos, pocas modificaciones son necesarias para mostrar la correctud de la variante más
general presentada en \cite{martello}. Cabe mencionar que en esta referencia solo se propone la
formulación y no se demuestra su correctud.
...
reducción al problema binario con dimensión $n_b \in \N$:
\begin{equation*}
	\max_{\vec{x} \in \braces{0, 1}^{n_b}} \braces{ \vec{\hat{p}}^T\vec{x} \vcentcolon \vec{\hat{p}}^T\vec{x} \leq
	u},
\end{equation*}
y para alguna $\vec{\hat{p}} \in \Z^{n_b}$ que puede ser obtenida de
$\vec{p}$ (ver página 82 de \cite{martello}). Observemos que si $u \geq
\sum_{i=1}^{n_b}\hat{p}_i$, entonces obtenemos la solución trivial $x_i = 1$ para todo $1 \leq i
\leq n_b$. Puesto que existen \textit{solvers} que implícitamente reducen problemas como
\eqref{theory:formulation} a su forma binaria (el ejemplo más famoso es \texttt{KnapsackSolver} de
Google OR-Tools), debemos ser cuidadosos con no introducir este tipo de trivialidades. Una forma simple
de evitar esta situación es exigir que el lado derecho de \eqref{theory:constraint:budget} satisfaga
$u < \sum_{i=1}^{n}p_i$.
...
\begin{equation*}
	...
	\end{cases}
\end{equation*}
para evitar obtener un problema trivial de acuerdo a la reducción binaria. Decidimos cambiar el
coeficiente de 0.5 a 0.1 para disminuir la probabilidad de que existan múltiples soluciones óptimas
en problemas de tamaño grande.

La tabla \ref{table:fin:mt:times} en la página \pageref{table:fin:mt:times} muestra los tiempos
promedios de terminación así como las desviaciones estándar de los métodos utilizados para realizar
cada experimento. La tabla \ref{table:fin:mt:cv} en la página \pageref{table:fin:mt:cv} muestra los
coeficientes de variación de estos tiempos. Un coeficiente de variación mide, en proporción, qué tan
concentrados están los tiempos alrededor de su media, y se calcula como $\sigma / \mu$, donde $\mu$
y $\sigma$ son la media y desviación estándar de los tiempos medidos, respectivamente.

De manera más inmediata, tenemos que los métodos diofantinos son significativamente más rápidos.
Además, los tiempos de terminación de los métodos diofantinos así como de la FPD están altamente
concentrados alrededor de la media, por lo que son estables. En efecto, los coeficientes de
variación de los métodos diofantinos estuvieron, en la gran mayoría de los casos, por debajo de
0.5\%, mientras que los de la FPD estuvieron por debajo de 1\%. En contraste, solo la mitad de los
coeficientes de variación de CBC estuvo por debajo del 1\%.

A pesar de que la FPD sea, en términos prácticos, igual de estable que los métodos diofantinos, no
debemos olvidar que la FPD no encontró una solución en el 61\% de los experimentos realizados.

Una de las hipótesis por las que creemos que \texttt{dioph\_left} presenta resultados aún más
...
segundo.
% Ciertamente podemos obtener equivalencias entre estos dos métodos al imponer un orden
...
\begin{table}[H]
	...
	\end{tabularx}
	\end{subtable}
	\caption{Tiempos de terminación promedio de los distintos métodos cuando varía la dimensión. Las
	casillas vacías indican que el método no encontró una solución para el problema correspondiente
	en menos de los 300 segundos de tolerancia. }
...
\begin{table}[H]
	...
	\ref{table:fin:mt:times}. Las casillas vacías indican que el método no encontró una solución
	para el problema correspondiente en menos de los 300 segundos de tolerancia. }
...
$\vec{p} \in \Z^n$ sea un vector aleatorio tomado de una distribución uniforme discreta y entera
sobre el intervalo $[10, 10{,}000)^n$. Luego, tomamos 128 puntos enteros aproximadamente
equidistantes sobre el intervalo $[.01S, S)$, donde $S \coloneq \sum_{i=1}^np_i$ y cada punto
representa el lado derecho de la restricción presupuestaria \eqref{theory:constraint:budget}.
Medimos los tiempos de terminación promedio sobre las 20 corridas. La figura
\ref{fig:exp:fin:rhs:zoom} muestra estos tiempos de terminación, mientras que la figura
\ref{fig:exp:fin:rhs:cv} muestra la distribución de sus coeficientes de variación.

\begin{figure}[hbtp]
	\centering
	\includegraphics[width=0.8\linewidth]{/home/sebastian/repos/thesis/static/fin/rhs/times-macro.pdf}
...
\begin{figure}[hbtp]
	\centering
	\includegraphics[width=0.7\linewidth]{/home/sebastian/repos/thesis/static/fin/rhs/boxplot-cv.pdf}
...
terminación de la FPD crecen linealmente con respecto al presupuesto. Aún así, destaca el hecho de que
la FPD resultó ser más consistente en sus tiempos de terminación que los otros tres métodos. Ambos FPD y
...
En ambos CBC y los métodos diofantinos hay presencia de un comportamiento oscilatorio. Se confirmó
para los métodos diofantinos que esto coincide en la mayoría de los casos cuando el presupuesto $u$
es múltiplo de una de las entradas de $\vec{q}$. En estos casos el problema es trivial, pues si $q_j
\mid u$, entonces uno de \texttt{dioph\_left} o \texttt{dioph\_right} encuentra inmediatamente la
solución $\vec{x} = \floor{u/q_j}\vec{e}_j$. En estos casos donde una entrada de $\vec{q}$ divide a
$u$, se observó que un repunte en \texttt{dioph\_left} coincide con una caída en
\texttt{dioph\_right} y viceversa. Las excepciones más notables a este comportamiento fueron las dos
observaciones anómalas en \texttt{dioph\_left} (ver figuras \ref{fig:exp:fin:rhs:zoom} y
\ref{fig:exp:fin:rhs:cv}). El comportamiento oscilatorio en CBC no coincide con los
casos en los que el presupuesto $u$ es múltiplo de alguna entrada de $\vec{p}$.

Observamos en la figura \ref{fig:exp:fin:rhs:cv} que la implementación de R\&A en CBC es sumamente
inestable. Se hipotetizó que esto se debe a la constante sobrecarga computacional de transferir
datos entre los módulos del problema relajado y la generación de cortes dentro de CBC. Por ello,
corrimos el mismo experimento tomando en cuenta una configuración de R\&A que prohíba la generación
de cortes. Debido a la dimensión $n = 1{,}000$ del problema y al fenómeno expuesto en la sección
\ref{sec:inf:exp}, no prohibimos el uso de heurísticas.

Se encontró que los tiempos de terminación son similares y por lo tanto no tiene sentido
...
configuración sin cortes. Es decir, en el 50\% de los 128 experimentos que realizamos, las
...
encontrar más rápidamente las soluciones óptimas. Al contrario, la generación de cortes provocó que
CBC tuviera una mayor varianza relativa en los tiempos de terminación.

En conclusión, consideramos que el argumento a favor del uso de métodos diofantinos es fuerte.
...
problema \eqref{theory:formulation} que sus contrapartes FPD o R\&A implementado en CBC. El
siguiente capítulo propone una extensión de estos métodos diofantinos para encontrar soluciones de
programas lineales enteros generales.

\begin{figure}[hbtp]
	\centering
	\includegraphics[width=0.7\linewidth]{/home/sebastian/repos/thesis/static/fin/rhs/boxplot-cv-wbb.pdf}
