...
\begin{proof}
	...
	\begin{equation*}
		...
		\theta_j\norm{\vec{u}_j}^2,
	\end{equation*}
	lo cual es no negativo para todo $1 \leq j \leq n$.

	Ahora mostramos la contención de derecha a izquierda por contraposición. Es decir, mostramos que
	si $\vec{x}$ no se encuentra en el lado izquierdo de \eqref{eq:sigmachar2}, entonces tampoco se
	encuentra en el lado derecho. Así pues, supongamos que $\vec{x} \not\in \sigma$. Por el lema
	\ref{lemma:sigmachar1} se sigue o bien que $\vec{x} \not\in \qlayer{\vec{q}}{k}$ o bien que
	$\vec{x} \not\in \R^n_{\geq \vec{0}}$. En el primer caso obtenemos inmediatamente que $\vec{x}$
	no se encuentra en el lado derecho de \eqref{eq:sigmachar2}. Luego, como $\vec{x} \not \in
	\sigma$, es válido suponer que $\vec{x} \in \qlayer{\vec{q}}{k}$ pero $\vec{x} \not\in \R_{\geq
	\vec{0}}$. Como $\braces{\vec{u}_i}_{i=1}^{n}$ es base de $\R^n$, existen escalares
...
$[\tau, \eta]$. Esta modificación no afecta en lo más mínimo la correctud o la complejidad del
algoritmo.
...
\ref{algo:fin:helper} no está expresado en forma de recursión terminal.
