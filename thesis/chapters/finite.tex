...
\begin{lemma}
	...
	\begin{equation*}
		\aff\braces{\vec{v}_1, \ldots, \vec{v}_m} = \vec{v}_j + \gen\braces{\vec{v}_i - \vec{v}_j}_{i \neq j}.
	\end{equation*}
\end{lemma}
\begin{proof}
	...
	\begin{align*}
		...
		&= \sum_{\substack{i=1 \\ i \neq j}}^{m}\theta_i(\vec{v}_i - \vec{v}_j) + \vec{v}_j.
	\end{align*}
	...
	\begin{equation*}
		\lambda_j = 1 - \sum_{\substack{i = 1 \\ i \neq j}}^{m}\lambda_i.
	\end{equation*}
	...
	\aff\braces{\vec{v}_1, \ldots, \vec{v}_m}$. Juntando esto con el punto anterior, obtenemos lo
...
\begin{example}
	...
	Si $\vec{q} > \vec{0}$ es un vector coprimo, entonces la $k$-ésima capa entera $H_{\vec{q},
	k\norm{\vec{q}}^{-2}}$ es la combinación afina de un conjunto de vectores. En efecto, recordemos
	de la Definición \ref{phase-1:def:c-layer} que esta capa entera es simplemente un hiperplano
	afino. Como $\vec{q}$ es el vector normal a esta capa entera, se sigue que puede ser escrito
	como $\vec{v} + \ker{\vec{q} \mapsto \vec{q}^T\vec{x}}$ para alguna $\vec{v} \in H_{\vec{q},
	k\norm{\vec{q}}^{-2}}$.
...
\end{definition}
\begin{observation}
	Todo símplice $\sigma$ generado por $\vec{v}_1, \ldots, \vec{v}_m$ está contenido en la
	combinación afina de estos vectores. Es decir,
	\begin{equation}
...
\begin{observation}
	...
	que las facetas constituyen las ``aristas'' de $\sigma$.
\end{observation}

\begin{lemma}
	\label{lemma:sigmachar1}
	Sea $\vec{q} \geq \vec{0}$ un vector coprimo y sea $H_{\vec{q}, k\norm{\vec{q}}^{-2}}$ la
...
entre el baricentro del símplice con cada una de sus facetas. Sabemos, bien por álgebra lineal, bien
por optimización, que la distancia entre $\sigma$ y su $j$-ésima faceta $\sigma_j$ es
\begin{equation}
	\label{eq:dist:v1}
...
\end{lemma}
\begin{proof}
	Sea $m \in \braces{1, \ldots, n} \setminus \braces{j}$. Como $\vec{u}_m \in \sigma_j$, debemos
	mostrar que $\vec{\mu}_j^T\vec{x} = 0$ para todo $\vec{x} \in \sigma_j - \vec{u}_m$.

	Tenemos de las definiciones \ref{def:aff} y \ref{def:simplex}, así como del Lema
	\ref{lemma:aff} que
	\begin{equation*}
	...
	\end{equation*}
	así que basta mostrar que $\vec{\mu}_j^T(\vec{u}_i - \vec{u}_m) = 0$ para todo $i \in \braces{1,
	\ldots, n}$ distinto de $j$ y $m$. Cabe mencionar que los vectores $\braces{\vec{u}_i}_{i=1}^{n}$
	son ortogonales entre sí. Sustituyendo con la definición de $\vec{\mu}_j$ en la hipótesis,
	obtenemos
	\begin{align*}
...
hecho de que el lado derecho \eqref{eq:signed} sea positivo. Geométricamente, lo anterior implica
que los vectores normales $\vec{\mu}_j$ de cada faceta $\sigma_j$ apuntan hacia adentro del símplice
$\sigma$. Esto sugiere una caracterización alternativa de $\sigma$ que nos permite interpretarlo
como un poliedro.
\begin{lemma}
	...
	\begin{equation}
		...
		\cap H_{\vec{q}, k\norm{\vec{q}}^{-2}},
	\end{equation}
	donde $\uvec{\mu}_j$ está definido en \eqref{eq:normal} y $\est{\sigma}_j$ es el baricentro de
	la $j$-ésima faceta de $\sigma$.
\end{lemma}
\begin{proof}
	...
	entrada negativa. Como $\braces{\vec{u}_i}_{i=1}^{n}$ es base de $\R^n$, existen escalares
...
\end{theorem}
\begin{proof}
	Como $\est{\sigma} \in \sigma$, tenemos del Lema \ref{lemma:sigmachar1} que
	...
	$\norm{\vec{x} - \est{\sigma}}_2 < r$. Entonces existe $\varepsilon > 0$ tal que
	\begin{equation*}
		\norm{(\vec{x} - \varepsilon\est{\mu}_j) - \est{\sigma}}_2 \leq r,
	\end{equation*}
	...
	\end{equation*}
	Pero $\vec{x} \in \sigma_j$ y del Lema \ref{mu:orth} sabemos que $\uvec{\mu}_j$ es un vector
	normal a $\sigma_j$. Es decir,
	\begin{equation*}
	...
	\begin{equation*}
		\vec{x} - \varepsilon\uvec{\mu}_j \not\in
		\bigcap_{j=1}^{n}
		\braces{\vec{x} \in \R^n \colon \uvec{\mu}_j^T(\vec{x} - \est{\sigma}_j) \geq 0}
		\cap H_{\vec{q}, k\norm{\vec{q}}^{-2}} = \sigma.
	\end{equation*}
	Donde la última igualdad sale del Lema \ref{lemma:sigmachar2}.
	Pero $\vec{x} - \varepsilon{\mu}_j \in B_r^{(k)}(\est{\sigma})$. De aquí se desprende que
	$B_r^{(k)}(\est{\sigma}) \not\subseteq \sigma$ si $r > d(\est{\sigma}, \sigma_j)$ para alguna $j
	\in \braces{1, \ldots, n}$. De la Definición \ref{def:r-sigma} obtenemos entonces
	\begin{equation}
		\label{r-sigma:up}
