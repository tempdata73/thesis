...
\end{equation}
Puesto que $q_{n-1} < 0 < q_n$, encontramos que las últimas dos entradas de
$\vec{x}$ son no negativas si y solo si $t_{n-1}$ satisface
\begin{equation}
...
$\vec{\nu}$ y $M$, determinar $\vec{t}$ y luego recuperar $\vec{x}$. El algoritmo \ref{algo:inf} en
la página \pageref{algo:inf} muestra este procedimiento constructivo. Para ello, suponemos la
existencia de una subrutina \texttt{Bezout} que, como su nombre lo indica, calcula los coeficientes
de Bézout entre dos enteros. Reiteramos, así como lo hicimos en la sección
...
El algoritmo \ref{algo:inf:ext} en la página \pageref{algo:inf:ext} extiende el algoritmo
\ref{algo:inf} en el sentido que incorpora esta forma de manejar las entradas nulas de $\vec{q}$.
Además, modifica el vector $\tvec{q}$ de manera que aseguramos que su penúltima entrada es negativa
y su última entrada es positiva, por lo que podemos deshacernos de este supuesto. Suponemos la
existencia de las subrutinas \texttt{length} y \texttt{swap} (véase capítulos 1 y 2 de
\cite{knuth1}), las cuales determinan la dimensión de un vector $\vec{q}$ y permutan dos de sus
...
\begin{algorithm}[H]
	...
	\SetKwFunction{NonNegativeIntSol}{NonNegativeIntSolInf}
		...
		\Begin{
			...
			$p \leftarrow 1$\quad\tcp*[h]{producto acumulado $g_1g_2\cdots g_{i+1}$}\;
...
La correctud de los dos algoritmos anteriores nos permite mostrar el siguiente teorema, el cual
postula que existe una ``reducción'' del problema \eqref{theory:formulation} al problema de resolver
una sola ecuación lineal diofantina. Cuando determinemos en la siguiente sección la complejidad de
ambos algoritmos, observaremos que el problema de resolver esta única ecuación lineal diofantina se
puede resolver en tiempo polinomial para un vector coprimo $\vec{q}$ fijo. En caso de que esta
reducción sea también polinomial, el problema \eqref{theory:formulation} se puede resolver
en tiempo polinomial con respecto a la dimensión del vector objetivo. A fin de evitar introducir
formalmente estos conceptos y arriesgar desviarnos por una tangente, no mostramos que esta reducción es,
en efecto, polinomial. Todo esto se cumple bajo el supuesto adicional de que el vector coprimo
$\vec{q}$ contenga al menos una entrada negativa.
...
\begin{figure}[hbtp]
  ...
  \begin{subfigure}{0.80\textwidth}
    \centering
    \includegraphics[width=\linewidth]{/home/sebastian/repos/thesis/static/inf/digits-bb_full.pdf}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.80\textwidth}
    \centering
    \includegraphics[width=\linewidth]{/home/sebastian/repos/thesis/static/inf/digits-dioph.pdf}
  \end{subfigure}
...
\begin{figure}[hbtp]
  ...
  \begin{subfigure}{0.80\textwidth}
    \centering
    \includegraphics[width=\linewidth]{/home/sebastian/repos/thesis/static/inf/mult-hist-cum-sep-v2.pdf}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.80\textwidth}
    \centering
    \includegraphics[width=\linewidth]{/home/sebastian/repos/thesis/static/inf/mult-hist-cum-v2.pdf}
  \end{subfigure}
