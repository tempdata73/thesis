...
\label{sec:inf:exp}
\noindent
Puesto a la simpleza de este capítulo que corresponde al primer caso del teorema
\ref{theory:th:feasibility}, el objetivo de esta subsección busca exhibir la dependencia implícita
que tiene Ramificación y Acotamiento con respecto a la magnitud del lado derecho de sus
desigualdades. Es común escuchar que la complejidad de este algoritmo depende exclusivamente de la
dimensión del problema así como de su número de desigualdades. Esto no es cierto y nos encargaremos
de mostrarlo en esta sección que se divide en tres pequeñas partes.

En primer lugar, determinamos la complejidad algorítmica del algoritmo \ref{algo:inf:ext}. En
segundo lugar, enseñamos por qué el método de R\&A también depende de la magnitud del lado derecho
de sus desigualdades y desarollamos las consecuencias que se desprenden de esto. En tercer lugar,
diseñamos un experimento numérico que ilustre esta dependencia de Ramificación y Acotamiento.

\subsection{Análisis de nuestro método}
\label{subsec:inf:complex}
\noindent
Primero analizamos el algoritmo \ref{algo:inf}. Para $1 \leq i \leq n - 2$, se sigue de la línea
\ref{alg:def:inf:g} que calculamos una vez el máximo común divisor de $n - (i + 1) + 1 = n - i$
números. Vimos en la subsección \ref{section:number-theory} que el máximo común divisor puede ser
definido de manera inductiva, por lo que calcular el máximo común divisor entre $n - i$ números es
equivalente a calcular $n - i$ veces el máximo común divisor entre dos números. Por lo tanto,
calculamos
\begin{equation*}
...
con el cálculo de aproximadamente $n^2$ máximo común divisor entre dos números, podemos ignorar, en
términos de complejidad, las llamadas a la subrutina \texttt{Bezout}.

Es posible mostrar que la complejidad de calcular el máximo común divisor entre dos enteros $a \leq
b$ es $\mathcal{O}(\log_2 b)$. Por lo tanto, la complejidad del algoritmo \ref{algo:inf} es
\begin{equation*}
	\mathcal{O}(n^2\log_2\norm{\vec{q}}_\infty) = \mathcal{O}(n^2),
\end{equation*}
puesto que nuestro vector coprimo $\vec{q}$ es constante.

Las subrutinas \texttt{length} y \texttt{switch} tienen complejidad constante. Además, recorremos el
vector $\vec{q}$ en el ciclo \ref{alg:inf:loop} una sola vez así como el vector $\tvec{x}$ en el
ciclo \ref{alg:inf:loop2}. Entonces el algoritmo \ref{algo:inf:ext} también tiene complejidad
cuadrática.

\subsection{Intuición de la dependencia con el presupuesto}
...
\begin{figure}[hbtp]
  ...
  \textit{Arriba}: Resultados tomando en cuenta Ramificación y Acotamiento (R\&A), así como la
	implementación dinámica. \textit{Abajo}: \textit{Zoom} en los métodos diofantinos.}
