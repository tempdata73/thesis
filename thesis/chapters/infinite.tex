...
$i$ cifras decimales, entonces queremos medir la proporción $T(\vec{p}_{i+1}) /
T(\vec{p}_i)$. Denominamos como multiplicadores a las razones anteriores, pues representan el factor
de aumento en los tiempos de terminación de R\&A cuando utilizamos una cifra decimal adicional.
Puesto que cada instancia depende del presupuesto $u$ de la restricción
\eqref{theory:constraint:budget} y tomamos 128 de estos presupuestos por cada nivel de precisión
$i$, obtenemos $128 \times (3 - 1) = 256$ multiplicadores registrados. La figura
\ref{fig:exp:inf:hist} muestra la distribución de estos multiplicadores, donde recortamos cada
multiplicador para que el máximo fuera 100, pues observamos que el más grande entre estos fue
aproximadamente 5{,}422.
