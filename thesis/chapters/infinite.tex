...
\begin{lemma}
	...
	El algoritmo \ref{algo:inf} de la página \pageref{algo:inf} es correcto.
\end{lemma}
\begin{proof}
...
la dimensión de un vector $\vec{q}$ y permutan dos de sus entradas, respectivamente. Ambas subrutinas son
...
\begin{theorem}
	\label{th:alg:inf}
	El algoritmo \ref{algo:inf:ext} de la página \pageref{algo:inf:ext} es correcto.
\end{theorem}
\begin{proof}
...
algoritmo \ref{algo:bb} del apéndice \ref{app:bb} (es decir, de una implementación ``pura'' de
Ramificación y Acotamiento) depende del número de decimales utilizados para especificar las entradas
del vector objetivo, así como de la magnitud en el lado derecho de las restricciones. En tercer
lugar, diseñamos un experimento numérico que ilustre esta dependencia.
...
\begin{example}
	...
	En la figura \ref{fig:infrec} se muestra el poliedro asociado a su problema relajado $S$.
	Encontramos que su solución es $(0.3, 0)$. Puesto que $0.3$ no es entero, ramificamos sobre esta
	variable y generamos dos subproblemas $S_{0}$ y $S_{1}$ con las restricciones añadidas $x \leq
	0$ y $x \geq 1$, respectivamente. La solución de $S_{0}$ es $(0, 0)$ y podamos por
	integralidad.

	Ahora bien, la solución de $S_{1}$ es $(1, 0.7)$ y como $0.7$ no es entero, generamos $S_{10}$
	y $S_{11}$ con las restricciones añadidas $y \leq 0$ y $y \geq 1$. Encontramos que el poliedro
	de $S_{10}$ es vacío, así que podamos por infactibilidad.

	En la parte derecha de la figura \ref{fig:infrec} se muestra el poliedro asociado a $S_{11}$.
	Observemos que este poliedro es una traslación del asociado a $S$. En otras palabras, $S_{11}$
	es un caso particular de una homotecia de $S$. Consecuentemente, hay un comportamiento periódico
	en cuanto a la ramificación y solución de estos problemas: resolver $S_{11}$ se reduce a
	resolver $S$, pero el primero es un subproblema del segundo. Siguiendo este razonamiento,
	encontramos que R\&A genera la cadena de subproblemas autosimilares
	\begin{equation*}
		S, S_{11}, S_{1111}, S_{111111}, \ldots
	\end{equation*}
	y el algoritmo \ref{algo:bb} del apéndice \ref{app:bb} jamás terminará con una solución.
\end{example}
...
Observemos que si el lado derecho hubiera sido $0$ en lugar de $0.3$, R\&A hubiera terminado
inmediatamente con la solución $(0, 0)$ al resolver el problema relajado. Es decir, un cambio de
$0.3$ unidades en el lado derecho de la única desigualdad provocó que el árbol generado por R\&A
tuviera profundidad unitaria a profundida infinita. Así pues, este ejemplo también muestra que
implementaciones puras de R\&A dependen de la magnitud del lado derecho de las restricciones.
...
$\vec{p} = m\vec{q}$, y donde $\vec{q}$ es el múltiplo coprimo de $\vec{p}$. Es decir, conjeturamos
...
\eqref{eq:cuts} y que se mantiene la factibilidad. El punto de intersección entre esta restricción junto con la restricción
presupuestaria \eqref{theory:constraint:budget} introduce un único vértice nuevo para el siguiente
subproblema relajado. Tenemos entonces del teorema \ref{prerreq:th:linear-sol} que este único vértice es una
solución. Los algoritmos más comúnes para resolver
...
relajado. Es
decir, si $\vec{x}^*$ es solución de un subproblema relajado, entonces $\big\lceil x_i^* \big\rceil$
constituye la $i$-ésima entrada de la solución del siguiente subproblema relajado obtenido al añadir el corte $x_i \geq
\big\lceil x_i^* \big\rceil$. En particular, encontramos que
\begin{equation}
	\label{eq:dembound}
	0 <
...
En la práctica no existen implementaciones puras de R\&A, sino que existe un ecosistema de módulos
que combinan R\&A con otro tipo de generación de cortes, con la manera de elegir subproblemas, con
métodos de presolución, con heurísticas, etcétera (ver figura \ref{p1c11:fig:MIP_solver_flowchart}).
Estas técnicas evitan, en primer lugar, que se genere un árbol de profundidad infinita y, en segundo
lugar, encuentran soluciones eficientemente.

En la siguiente subsección mostramos que la obtención de soluciones de \eqref{theory:formulation}
todavía es lenta cuando una entrada del vector coprimo $\vec{q}$ del vector objetivo $\vec{p}$ es
negativa.
...
por medio de la interfaz de PuLP 3.1 implementada en Python 3.12. La configuración
usada permite métodos de presolución, cortes de mochila y de Gomory-Chvátal. Prohibimos el uso de
...
fueran distintos. Cabe mencionar que nuestra implementación se encuentra libre en
GitHub\footnote{Véase: \url{https://github.com/tempdata73/thesis}.}
...
con cinco cifras decimales. Evidentemente, los vectores coprimos $\vec{q}_4$ y $\vec{q}_5$ asociados
respectivamente a $\vec{p}_4$ y $\vec{p}_5$ tienen entradas negativas en los mismo lugares que
$\vec{q}_3$.
...
corridas. Las rupturas de continuidad en las líneas son causadas porque R\&A no encontró una
solución antes del límite de 300 segundos.
...
En realidad nos interesa mostrar la sensibilidad en los tiempos de terminación de R\&A a medida que
la precisión del vector objetivo $\vec{p}$ aumenta. Es decir, si $T(\vec{p}_i)$ es el tiempo de
terminación de R\&A con una instancia usando el vector esencialmente entero $\vec{p}_i$ que tiene
...
los tiempos de terminación de R\&A cuando utilizamos una cifra decimal adicional. Puesto que cada
...
300 segundos. La figura \ref{fig:exp:inf:hist} muestra la distribución acumulada de estos
multiplicadores. Por cuestiones visuales, recortamos la gráfica para que el multiplicador más grande
fuera 100, pues 49 multiplicadores se encontraban en el rango (100, 5{,}422] y
generaban un sesgo innecesario en la distribución.
...
conteo en los archivos de registro generados por el \textit{solver} CBC, descubrimos que en
\textbf{todos} los casos que R\&A encontró una solución fue a causa de una heurística y no por el
método de ramificación. Debido la dificultad del árbol que genera el problema
...
Esperamos que lo realizado en estos experimentos numéricos muestre categóricamente que el método de
Ramificación y Acotamiento no depende solo del número de variables o de desigualdades utilizadas,
sino que también de la precisión numérica con la que se especifican los datos. Con esto damos por
concluido este capítulo y continuamos con nuestro desarrollo del segundo caso del teorema
...
\begin{figure}[hbtp]
  ...
  \begin{subfigure}{0.80\textwidth}
    \centering
    \includegraphics[width=\linewidth]{/home/tempdata/repos/thesis/static/inf/mult-hist-cum-sep.pdf}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.80\textwidth}
    \centering
