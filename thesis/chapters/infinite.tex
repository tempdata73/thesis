...
\begin{proof}
	...
	% Cuando $i = 1$, tenemos $\vec{q} = \vec{q}'$. De la línea \eqref{alg:def:inf:g}, obtenemos $g_2
	...
	% \end{equation*}
	% pero esto es igual a la definición en \eqref{dummy:next-g}. De la línea \eqref{alg:def:inf:bez}
	% tenemos que $x_1'$ y $\omega_2'$ son los coeficientes de Bézout de $q_1 = q_1'$ y de $g_2$,
	% respectivamente. De la línea \eqref{alg:def:inf:w} tenemos
	% \begin{equation*}
	...
	% De las redefiniciones en la línea \eqref{alg:def:inf:q} obtenemos, para todo $j \in \braces{2,
	% \ldots, n - 1}$,
	% \begin{equation*}
	...
	% línea \eqref{alg:def:inf:q} tenemos entonces
	% \begin{equation*}
	% 	q_j \leftarrow \frac{q_j}{g_i} = \frac{q_j'}{g_i \cdot \prod_{j=1}^{i-1} g_j}
	% 	= \frac{q_j'}{\prod_{j=1}^{i}g_j},
	% \end{equation*}
	% para toda $j \in \braces{i, \ldots, n - 1}$. De la línea \eqref{alg:def:inf:g} se sigue que
	% \begin{equation*}
	...
	% 	},
	% \end{equation*}
	% lo cual es equivalente a \eqref{dummy:next-g}. De la línea \eqref{alg:def:inf:bez} tenemos que
	...
	% satisfacen \eqref{dummy:eq:simplified}. Observamos en la línea \eqref{alg:def:inf:t} que $t_i$
...
\begin{algorithm}[ht]
	...
	\Fn{\Dioph{$\vec{q}$, $\eta$}}{
		...
		$m \leftarrow$ \length{$\vec{\tilde{q}}$}\; \label{alg:subr:length}
		\switch{$\vec{\tilde{q}}$, $1$, $m$}\; \label{alg:subr:switch1}

		\For{$i \leftarrow 1$ \KwTo $m - 1$}{ \label{alg:inf:loop}
			\If{$\tilde{q}_i < 0$}{
				$j \leftarrow i$\;
				ir al paso \eqref{alg:subr:switch3}\;
			}
		}
		\switch{$\vec{\tilde{q}}$, $j$, $m - 1$}\; \label{alg:subr:switch3}

		$\vec{\tilde{x}} \leftarrow$ \NonNegativeIntSol{$\vec{\tilde{q}}$, $\eta$}\;
		\switch{$\vec{\tilde{x}}$, $j$, $m - 1$}\; \label{alg:subr:switch2}
		\switch{$\vec{\tilde{x}}$, $1$, $m$}\;
