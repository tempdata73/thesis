...
\begin{proof}
	...
	Finalmente, en el ciclo que comienza en la línea \ref{alg:inf:loop2} insertamos en $\vec{x}$ las entradas $i$
...
\begin{example}
	...
	homotecia de $S_{0}$. Consecuentemente, hay un comportamiento
...
$u$ de \eqref{theory:constraint:budget} es tal que $u/m \not\in \Z$, donde $m$ es el escalar que
satisface $\vec{p} = m\vec{q}$, y donde $\vec{q}$ es el múltiplo coprimo de $\vec{p}$. En
particular, para problemas de este tipo existe una colección de subproblemas homotéticos entre sí.
Además, si el vector coprimo $\vec{q}$ de $\vec{p}$ tiene una entrada negativa, entonces esta
colección es infinita. Por cuestiones de longitud de esta tesis, no investigamos más en este
respecto.
...
300 segundos. La figura \ref{fig:exp:inf:hist} muestra la distribución de estos multiplicadores,
donde recortamos cada multiplicador para que el máximo fuera 100, pues observamos que el más grande
entre estos fue aproximadamente 5{,}422.
...
conteo en los archivos de registro generados por el \textit{solver} CBC, el autor descubrió que en
