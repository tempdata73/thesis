...
en alguna capa entera $\qlayer{q}{k}$ con parámetro entero $k \leq \eta$.
...
\begin{proof}
	...
	\end{align*}
	es una descomposición de $\Z^n$ entre las redes $\Lambda_p \oplus \Lambda_h$ definidas en
	\eqref{eq:dec}. Por consiguiente,
	\begin{align*}
	...
	es una cota inferior de \eqref{formulation:multiple} es análoga.

	Ahora mostramos por contradicción que podemos recuperar la solución de un problema a partir de
	la solución del otro. Supongamos que $(k, \vec{t}) \in \Z^n$ es solución de
	\eqref{formulation:lattice}. Si existe $\tvec{x}$ factible para \eqref{formulation:multiple} con
	utilidad $\vec{q}^T\tvec{x} = \tilde{k}$ estrictamente mayor, entonces consideramos ($\tilde{k},
	\tvec{t})$ tal que $\tvec{x} = \tilde{k}\vec{\nu} + M\tvec{t}$. Este vector también es factible
	con utilidad $k < \tilde{k} \leq \eta$, y entonces $(k, \vec{t})$ no era la solución de
...
% a la restricción \eqref{lattice:c-layer}.

Este problema equivalente induce a que las políticas de poda en Ramificación y Acotamiento sean más
eficientes. En efecto, la consecuencia principal del siguiente teorema es que siempre es mejor
priorizar ramificaciones en $k$ puesto que eliminamos de manera inmediata subproblemas
infactibles.
\begin{theorem}
	\label{th:bbspeed}
...
\begin{subequations}
	...
	\begin{align}
		...
		\text{s.a.} \quad
		k &\leq \eta, \\
			A\tvec{y} &= \vec{b} - kA\vec{\nu} \label{subformulation:lattice:rest},
	\end{align}
\end{subequations}
donde 
\begin{equation}
...
\begin{equation}
	...
	= \vec{b} - kA\vec{\nu},
\end{equation}
comparando con la restricción \eqref{subformulation:lattice:rest} y la definición de $\tvec{y}$ en
\eqref{eq:tvecy}, esto sugiere definir
\begin{equation}
	\label{eq:ym}
...
cada $k \leq \eta$ entera, la correspondiente solución $\tvec{y}_m$ es entera o no.
...
\begin{equation}
	...
	= SU_m\tvec{y}_{m} \eqcolon \vec{z}.
\end{equation}
Si $D_{ii}$ no divide a $z_i$ para alguna $i \in \lbrace 1, \ldots, n \rbrace$,
...
Supongamos, pues, que $D_{ii} \mid z_i$ para todo $i \in \lbrace 1, \ldots, n\rbrace$, por lo que
obtenemos un vector $\vec{r} \in \Z^n$ de soluciones enteras con $r_i \coloneq z_i / D_{ii}$ y otro
vector $\vec{s} \in \Z^{n-m-1}$ de variables libres:
\begin{equation*}
	\inv{T}\begin{pmatrix} \vec{t} \\ \tvec{y}_{n-m} \end{pmatrix}
	=
	\begin{pmatrix} \vec{r} \\ \vec{s} \end{pmatrix}.
\end{equation*}
Por lo tanto, nuestro vector $\vec{t}$ es una función afín de $\vec{s}$, es decir, $\vec{t} =
...
Consideremos el conjunto de vectores $\vec{s} \in \Z^{n-m-1}$ que obligan a que $\vec{t}(\vec{s})$
...
divisores. En \cite{knuth1}, \cite{alex} y \cite{morris} se muestra que realizar este tipo de
cálculos, así como de obtener las formas normales de Hermite y de Smith, son problemas acotados en
tiempo polinomial.
...
\ref{ex:two-var}. En segundo lugar, si $m = n - 2$, obtenemos un solo parámetro libre $s \in \Z$,
con lo que podemos determinar rápidamente la existencia o inexistencia de un conjunto de
factibilidad en $s$ que obliga a que $\vec{t}(s)$ satisfaga \eqref{subf:feas:ineq}. Este caso se
ilustra en el ejemplo \ref{ex:three-var}.

A modo de resumen, mostramos en el pseudocódigo \ref{algo:multiple} en la página
\pageref{algo:multiple} la forma de resolver problemas del tipo \eqref{formulation:lattice}. Por el
teorema \ref{th:multeq}, este método también resuelve problemas del tipo
\eqref{formulation:multiple}. Después de presentar los ejemplos \ref{ex:two-var} y
\ref{ex:three-var}, mostramos una manera con la cual podemos deshacernos del ciclo infinito en la
línea \ref{algo:mult:infloop} de este pseudocódigo.
...
\begin{algorithm}[H]
	...
	\Begin{
		...
		\For{$i \leftarrow 1$ \KwTo $n$}{
			\If{$D_{ii}$ \textnormal{no divide a} $z_i$}{
				$k \leftarrow k - 1$\;
				ir a la línea \ref{algo:mult:infloop}\;
			}
			$r_i \leftarrow z_i / D_{ii}$\;
		}
		$(\vec{t}(\vec{s}), \tvec{y}_{n-m}(\vec{s})) \leftarrow T(\vec{r}, \vec{s})^T$\; \label{algo:mult:free}
		\If{\textnormal{existe} $\vec{s}$ \textnormal{tal que} $M\vec{t}(\vec{s}) \geq -k\vec{\nu}$\label{algo:mult:hard}}{
...
\newpage
Para calcular las formas normales de Hermite y de Smith de la matriz de restricciones $A \in
\Z^{m \times n}$ de los siguiente ejemplos, utilizamos la librería \texttt{hsnf} de
Python\footnote{
...
\begin{example}
	...
	Observemos que $D_{22}$ no divide a $z_2$, y entonces el subproblema \eqref{subformulation:feasibility}
...
\begin{example}
	...
	\end{equation*}
	de donde se sigue inmediatamente que $-5 \leq s \leq 0$. Sustituyendo
	cada valor entero de $s$ en $\vec{t}(s)$ y transformando a $\vec{x}^*(s) = k^*\vec{\nu} +
	M\vec{t}(s)$, tenemos que
	\begin{equation*}
		\left\lbrace
...
En primer lugar, si $\eta = \infty$, entonces el problema relajado es no acotado. En el teorema 1 de
\cite{goldman} se demuestra que si esto ocurre y además las entradas de $A$, de $\vec{b}$ y de
$\vec{q}$ son todas racionales, entonces el programa lineal entero o es infactible o es no acotado.
Como tenemos integralidad de $A$ y de $\vec{b}$ por suposición, así como integralidad de $\vec{q}$
por la definición \ref{theory:def:rational}, no tiene sentido usar el pseudocódigo
\ref{algo:multiple} para buscar una solución de este problema, independientemente de que el
problema sea infactible o no acotado.
...
\ref{algo:multiple} eventualmente terminará con una solución de este problema.
...
\ref{algo:multiple} termine. Observemos que, en este caso, existe la posibilidad de que $P \neq
\emptyset$ pero $P \cap \Z^n = \emptyset$. Sin realizar modificaciones grandes al pseudocódigo
\ref{algo:multiple}, encontramos que, o bien termina con una solución $\vec{x}^*$ del problema
...
método, en cambio, solo necesita resolver dos problemas relajados.

A modo de conclusión de este capítulo, mencionamos que futuras líneas de
investigación podrían estar concentradas en resolver el problema de la línea \ref{algo:mult:hard}
del pseudocódigo \ref{algo:multiple}. Esto se reduce a investigar sistemas de desigualdades lineales
en los enteros. Existen tres posibilidades para estas investigaciones con respecto al vector de
variables libres $\vec{s} \in \Z^{n-m-1}$:
\begin{enumerate}
...
desarrollar las consecuencias del teorema \ref{th:bbspeed}. Es nuestra creencia que los
